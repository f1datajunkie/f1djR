...
plist=intersect(c(results$code),unique(bmLapTimes$code))
```

```{r race_reportloop}
output_files = c('* [Race](race/README.md)')

#Fastlaps
outfile = paste0(rootdir,'/',outdir,'/race_report_fastlaps.md')
rmarkdown::render('fastlaps_template.Rmd',
       params = list(stub=stub, outdir = outdir, rootdir=rootdir,
                     ...
                     results=results), output_file = outfile)
output_files = c(output_files, paste0('\n\t* [Fastest Laps](', outdir, '/', outfile, ')' ))

#Lap analysis
outfile = paste0(rootdir,'/',outdir,'/race_report_lap_analysis.md')
rmarkdown::render('lap_analysis_template.Rmd',
       params = list(stub=stub, outdir = outdir, rootdir=rootdir,
                     year = year, country=country,session=session,
                     lapTimes=lapTimes, results=results,
                     pitstops=pitstops), output_file = outfile)
output_files = c(output_files, paste0('\n\t* [Lap Analysis](', outdir, '/', outfile, ')' ))


#Track Concordance
outfile = paste0(rootdir,'/',outdir,'/race_track_concordance.md')
rmarkdown::render('race_track_concordance_template.Rmd',
       params = list(stub=stub, outdir = outdir, rootdir=rootdir,
                     year = year, country=country,session=session,
                     plist=plist), output_file = outfile)
output_files = c(output_files, paste0('\n\t* [Track Concordance](', outdir, '/', outfile, ')' ))



#Battlemaps
outfile = paste0(rootdir,'/',outdir,'/race_battlemaps.md')
...
outfile = paste0(rootdir,'/',outdir,'/race_history_track_position.md')
rmarkdown::render('race_history_track_position_template.Rmd',
       params = list(stub=stub, outdir = outdir, rootdir=rootdir,
                     year = year, country=country,session=session,
                     lapTimes=lapTimes), output_file = outfile)
...
#Pitstop analysis
outfile = paste0(rootdir,'/',outdir,'/race_report_pitstop_analysis.md')
