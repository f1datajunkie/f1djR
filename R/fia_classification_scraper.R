...
fiaSessionClassPracticeTidy=function(xx){
  xx['laptime']=apply(xx['TIME'],1,timeInS2)
  xx
}
...
fiaSectorTidy=function(xx){
  ...
  for (s in c('s1','s2','s3')) {
    ...
    xx[sn]=as.numeric(apply(xx[sm],1,timeInS2))
  }
...
fiaRaceFastlapTidy=function(xx){
  xx['time']=apply(xx['LAP TIME'],1,timeInS2)
  xx
}
...
fiaPitsSummary=function(xx){
  ...
  xx['tot_time']=apply(xx['TOTAL TIME'],1,timeInS2)
...
fiaPitsDetail=function(xx){
  ...
  xx['tot_time']=apply(xx['TOTAL TIME'],1,timeInS2)
  xx['time']=apply(xx['DURATION'],1,timeInS2)
  r=which(xx['NO']=='RACE - PIT STOP - DETAIL')
  xx=xx[r+2:nrow(xx),]
