...
lapsData.df=function(year,raceNum,format='json',con=NA){
  if (is.na(con)) {
    ...
    q=paste('SELECT lap,driverRef as driverId, position, time as strtime,
            1.0*milliseconds/1000 as rawtime FROM lapTimes AS l JOIN drivers AS d JOIN races AS r
            WHERE r.year="',year,'" AND r.round="',raceNum,'" AND r.raceId=l.raceId
            AND l.driverId=d.driverId',sep='')
    ld=dbGetQuery(con,q)
  }

  lap.data=ddply(ld,.(driverId),transform,cuml=cumsum(rawtime))
...
formatPitsData=function(rd.pits){
  ...
  for (i in 1:length(pd)){
    pits.data=rbind(pits.data,data.frame(
      ...
      milliseconds=1000 * as.numeric(timeInS(pd[[i]]$duration))
    ))
  }
  pits.data
}
...
#' @parameter character url URL of the API call
#' @return dataframe containing qualifying results
qualiResults.df=function(year=NA,raceNum=NA,driverRef=NA,constructorRef=NA,format='json',con=NA){
  if (is.na(con)){
    url=getQuali.URL(year=year,
                   ...
                   format=format)
    quali.results.data=qualiResultsParse.df(url)
  } else {
    #Need to add driverRef and construcotRef filters
    q=paste('SELECT d.driverRef AS driverId, d.code AS code, q.position,
            q1 AS Q1, q2 AS Q2, q3 AS Q3
            FROM qualifying AS q JOIN drivers AS d JOIN races AS r
            WHERE r.year="',year,'" AND r.round="',raceNum,'" AND r.raceId=q.raceId
            AND q.driverId=d.driverId',sep='')
    quali.results.data=dbGetQuery(con,q)
    for (q in c('Q1','Q2','Q3')){
      quali.results.data[paste(q,'_time',sep='')]=as.numeric(apply(quali.results.data[q],1,timeInS))
    }
  }

  for (q in c('Q1','Q2','Q3')){
    quali.results.data[paste(q,'_rank',sep='')]=rank(quali.results.data[paste(q,'_time',sep='')],
...
resultsData.df=function(year,raceNum){
  ...
  for (i in 1:length(rrd)){
    race.results.data=rbind(race.results.data,data.frame(
      ...
      code=as.character(driverCodeExists(rrd[[i]]$Driver)),
  ...
  for (i in 1:length(drd)){
    for (j in 1:length(drd[[i]]$DriverStandings))
      driverStandings.data=rbind(driverStandings.data,data.frame(
        ...
        code=as.character(driverCodeExists(drd[[i]]$DriverStandings[[j]]$Driver)),
